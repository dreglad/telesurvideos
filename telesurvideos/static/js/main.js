...
var offcanvasMenu = function () {
    ...
    $('.offcanvas-has-dropdown>a').append('<i class="icon-chevron-down pull-right"></i>');
    // Hover dropdown menu on mobile

    $('.offcanvas-has-dropdown>a>i').click(function (ev) {
        ev.preventDefault();
        var $li = $(this).parent().parent();
        var $this = $(this);
        if (!$li.hasClass('active')) {
            $this.toggleClass('icon-chevron-down icon-chevron-up');
            var h_dr = $li.find('.dropdown').height() + 40;
            $li
                .addClass('active')
                .css('height', h_dr)
                .find('ul')
